...
import { MyDocumentProps } from "./types";
...
const ServiceAgreement: FC<MyDocumentProps> = ({ data }) => {
  const { customer, contract, company } = data;
  ...
  const sectionContract = {
    ...
    price: [
      ...
      `3.2. Оплата услуг производится в следующем порядке: ${
        contract.paymentMethod === "наличными"
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.headerLocation}>
            <Text>Город {contract.city}</Text>
            <Text>{`"${contract.date.getDate()}" ${
              MONTH[contract.date.getMonth()]
            } ${contract.date.getFullYear()}`}</Text>
          </View>
        </View>
        <View>
          {/* aggreement */}
          ...
          <View style={styles.item}>
            ...
            <TextList data={sectionContract.item} />
          </View>
